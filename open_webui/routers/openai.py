...
async def generate_chat_completion(
    ...
    # Qwen 3.6 thinking control: default OFF, toggled ON by Think button
    # Think button sets system prompt to "/think"; we strip it and set the real flag
    _thinking_on = False
    if isinstance(payload.get('messages'), list):
        for _msg in payload['messages']:
            if _msg.get('role') == 'system':
                _sys = _msg.get('content', '')
                if '/think' in _sys:
                    _thinking_on = True
                    _msg['content'] = _sys.replace('/think', '').strip()
                if not _msg['content']:
